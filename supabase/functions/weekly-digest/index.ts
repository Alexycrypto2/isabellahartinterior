...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Check if digest is enabled via site_settings
    ...
    const emailHtml = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 640px; margin: 0 auto; padding: 24px; background: #ffffff;">
        <div style="text-align: center; margin-bottom: 24px;">
          <h1 style="color: #333; font-size: 24px; margin: 0 0 4px;">📊 Weekly Performance Digest</h1>
          <p style="color: #888; font-size: 14px; margin: 0;">${weekStart} — ${weekEnd}</p>
        </div>

        <!-- Summary Cards -->
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 28px;">
          <tr>
            <td style="width: 33%; text-align: center; padding: 16px; background: #f9f9f9; border-radius: 8px;">
              <div style="font-size: 28px; font-weight: 700; color: #d4a853;">${totalPageViews ?? 0}</div>
            ...
            <td style="width: 33%; text-align: center; padding: 16px; background: #f9f9f9; border-radius: 8px;">
              ...
              <div style="font-size: 12px; color: #888; margin-top: 4px;">Affiliate Clicks</div>
            </td>
            <td style="width: 8px;"></td>
            <td style="width: 33%; text-align: center; padding: 16px; background: #f9f9f9; border-radius: 8px;">
              <div style="font-size: 28px; font-weight: 700; color: #d4a853;">${totalBlogViews}</div>
              <div style="font-size: 12px; color: #888; margin-top: 4px;">Blog Views</div>
            </td>
          </tr>
        </table>

        <!-- Top Products -->
        <h2 style="color: #333; font-size: 18px; border-bottom: 2px solid #d4a853; padding-bottom: 8px; margin-bottom: 12px;">🛒 Top Products by Clicks</h2>
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 28px;">
          <thead>
        ...
        <!-- Top Blog Posts -->
        <h2 style="color: #333; font-size: 18px; border-bottom: 2px solid #d4a853; padding-bottom: 8px; margin-bottom: 12px;">📝 Top Blog Posts by Views</h2>
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 28px;">
          <thead>
            <tr style="background: #fafafa;">
              <th style="padding: 8px 12px; text-align: left; color: #888; font-size: 12px; font-weight: 600;">#</th>
        ...
        <div style="text-align: center; padding-top: 16px; border-top: 1px solid #eee;">
          <p style="color: #999; font-size: 12px; margin: 0;">Sent from RoomRefine Weekly Digest</p>
        </div>
      </div>
    `;

    // Resolve email: digest override → contact notification_email → contact email → env fallback
    ...
    const { error: sendError } = await resend.emails.send({
      ...
      to: [siteEmail],
      subject: `📊 Weekly Digest: ${totalClicks} clicks, ${totalBlogViews} blog views (${weekStart} – ${weekEnd})`,
      html: emailHtml,
    });
